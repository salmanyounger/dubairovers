    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { emoji:'💰', label:'Platform Revenue',   value:`AED ${totalRevenue.toLocaleString('en-US')}`,       color:'#D4AF37' },
          { emoji:'💸', label:'Commissions Paid',   value:`AED ${totalCommission.toLocaleString('en-US')}`,    color:'#8B5CF6' },
        ].map(s=>(
            ...
            <tbody>
              {partners.map((p,i)=>(
                <tr key={p.id}
                  ...
                  <td className="px-4 py-3 font-black text-brand-navy text-sm">AED {p.revenue.toLocaleString('en-US')}</td>
                  <td className="px-4 py-3 font-bold text-emerald-600">AED {p.commission.toLocaleString('en-US')}</td>
                  <td className="px-4 py-3">
        ...
        <div className="fixed inset-y-0 right-0 w-80 bg-white z-50 shadow-2xl overflow-y-auto"
          ...
          <div className="p-5 space-y-3">
            {[
              ...
              ['💰 Revenue', `AED ${selected.revenue.toLocaleString('en-US')}`],
              ['💸 Commission', `AED ${selected.commission.toLocaleString('en-US')}`],
