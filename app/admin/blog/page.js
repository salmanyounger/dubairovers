...
export default function AdminBlog() {
  ...
  // ── Bulk schedule — N per day at 9am Dubai time ─────────────
  const [scheduleRate, setScheduleRate] = useState(5); // blogs per day

  const bulkSchedule = () => {
    if (!confirm(`Schedule ${stats.drafts} drafts at ${scheduleRate} per day starting 9am Dubai time?`)) return;
    const now = new Date();
    // Find start offset after last scheduled blog
    const lastScheduled = blogs
      .filter(b => b.status === 'scheduled')
      .sort((a, b) => new Date(b.publishAt) - new Date(a.publishAt))[0];
    let dayOffset = 1;
    if (lastScheduled) {
      const lastDate = new Date(lastScheduled.publishAt);
      dayOffset = Math.max(1, Math.ceil((lastDate - now) / 86400000) + 1);
    }
    let slotIndex = 0; // position within current day (0 to scheduleRate-1)
    const updated = blogs.map(b => {
      if (b.status !== 'draft') return b;
      const currentDay = dayOffset + Math.floor(slotIndex / scheduleRate);
      const d = new Date(now);
      d.setDate(d.getDate() + currentDay);
      d.setHours(5, 0, 0, 0); // 9am Dubai = 5am UTC
      slotIndex++;
      return { ...b, status: 'scheduled', publishAt: d.toISOString() };
    });
    saveBlogs(updated);
    const daysNeeded = Math.ceil(stats.drafts / scheduleRate);
    toast(`🗓️ ${stats.drafts} blogs scheduled — ${scheduleRate}/day · done in ${daysNeeded} days!`);
    window.dispatchEvent(new CustomEvent('dr_data_update', { detail: { key:'blog_posts', data:updated } }));
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <select value={scheduleRate} onChange={e=>setScheduleRate(+e.target.value)}
              className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 text-brand-navy font-bold">
            ...
            <button onClick={bulkSchedule} className="btn-gold text-xs px-3 py-1.5" title="Schedule all drafts at selected rate">{`🗓️ Auto-Schedule ${scheduleRate}/day`}</button>
          </div>
                ...
                return (
                  <tr key={blog.id} className={`border-b border-gray-50 hover:bg-amber-50/20 transition-colors ${i%2?'bg-gray-50/20':''}`}>
                    ...
                    <td className="px-4 py-3 text-xs font-semibold text-gray-500">
                      {blog.views > 0 ? `${blog.views.toLocaleString('en-US')}` : '—'}
                    </td>
                    <td className="px-4 py-3">
