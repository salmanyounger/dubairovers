    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { emoji:'💰', label:'Total Paid Out',    value:`AED ${totalEarnings.toLocaleString('en-US')}`,           color:'#D4AF37' },
            ...
            <tbody>
              {filtered.map((a,i)=>(
                <tr key={a.id} className={`border-b border-gray-50 hover:bg-amber-50/30 transition-colors cursor-pointer ${
                  ...
                  <td className="px-4 py-3 font-black text-brand-navy">AED {a.earnings.toLocaleString('en-US')}</td>
                  <td className="px-4 py-3 font-semibold text-center">{a.bookings}</td>
                  <td className="px-4 py-3 text-gray-500 text-center">{a.clicks.toLocaleString('en-US')}</td>
        ...
        <div className="fixed inset-y-0 right-0 w-80 bg-white z-50 shadow-2xl overflow-y-auto"
          ...
          <div className="p-5 space-y-4">
            ...
            {[
              ...
              ['🔗 Link', selected.link],
              ['💰 Earnings', `AED ${selected.earnings.toLocaleString('en-US')}`],
              ['📋 Bookings', selected.bookings],
              ['👆 Clicks', selected.clicks.toLocaleString('en-US')],
