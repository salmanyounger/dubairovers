  ...
  const fmt=(text)=>{
    ...
    return text.split("\n\n").map((para,i)=>{
      ...
      const parts=para.split(/(\[[^\]]+\]\([^)]+\)|\*\*[^*]+\*\*|`[^`]+`)/g).map((pt,j)=>{
        ...
        if(pt.startsWith("`")&&pt.endsWith("`")) return <code key={j} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:12,color:T.green,background:"rgba(16,185,129,0.08)",padding:"1px 5px",borderRadius:4}}>{pt.slice(1,-1)}</code>;
        return pt;
      });
