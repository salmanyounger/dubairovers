...
export default function SalmanFXBlogPost() {
  const params  = useParams();
  const slug    = params?.slug;
  const [mounted, setMounted]       = useState(false);
  const [adminPosts, setAdminPosts] = useState([]);

  useEffect(()=>{ setMounted(true); try{const s=JSON.parse(localStorage.getItem("sfx_posts")||"[]"); setAdminPosts(s);}catch(_){} },[]);
  useEffect(()=>{
    if(!mounted) return;
    const post=POSTS[slug]||adminPosts.find(p=>p.slug===slug);
    if(!post) return;
    const schema={"@context":"https://schema.org","@type":"BlogPosting","headline":post.title,"description":post.desc,"keywords":post.keywords||post.tag,"datePublished":post.isoDate||"2026-01-01","dateModified":post.isoDate||"2026-01-01","author":{"@type":"Person","name":"Salman Ali","url":`${SITE}/salmanfx`},"publisher":{"@type":"Organization","name":"SalmanFX","url":`${SITE}/salmanfx`},"mainEntityOfPage":{"@type":"WebPage","@id":`${SITE}/salmanfx/blog/${slug}`},"inLanguage":"en-US"};
  ...
  if(!post) return (
    <div style={{minHeight:"100vh",background:"#060A0D",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      <div style={{textAlign:"center"}}>
        ...
        <Link href="/salmanfx/blog" style={{padding:"10px 24px",background:"linear-gradient(135deg,#10B981,#059669)",borderRadius:20,color:"#000",fontSize:13,fontWeight:700,textDecoration:"none"}}>← Back to Blog</Link>
      </div>
    </div>
  );

  const T={bg:"#060A0D",border:"rgba(16,185,129,0.12)",text:"#D4F0E8",sub:"rgba(255,255,255,0.4)",green:"#10B981"};
  ...
  const fmt=(text)=>{
    ...
    return text.split("\n\n").map((para,i)=>{
      if(para.startsWith("**")&&para.endsWith("**"))
        return <h3 key={i} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:18,fontWeight:700,color:"#fff",margin:"26px 0 10px",borderLeft:"3px solid #10B981",paddingLeft:12}}>{para.replace(/\*\*/g,"")}</h3>;
      const parts=para.split(/(\[[^\]]+\]\([^)]+\)|\*\*[^*]+\*\*|`[^`]+`|'[A-Za-z0-9_.()\s,\[\]<>|#]+?')/g).map((pt,j)=>{
        if(pt.startsWith("[")&&pt.includes("](")){ const txt=pt.match(/\[([^\]]+)\]/)?.[1]; const url=pt.match(/\(([^)]+)\)/)?.[1]; const ext=url?.startsWith("http"); return ext?<a key={j} href={url} target="_blank" rel="noopener noreferrer" style={{color:T.green,textDecoration:"underline",textUnderlineOffset:2}}>{txt}</a>:<Link key={j} href={url||"/"} style={{color:T.green,textDecoration:"underline",textUnderlineOffset:2}}>{txt}</Link>; }
        if(pt.startsWith("**")&&pt.endsWith("**")) return <strong key={j} style={{color:"#fff",fontWeight:700}}>{pt.replace(/\*\*/g,"")}</strong>;
        if((pt.startsWith("`")&&pt.endsWith("`"))||(pt.startsWith("'")&&pt.endsWith("'")&&pt.length>3)) return <code key={j} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:12,color:T.green,background:"rgba(16,185,129,0.08)",padding:"1px 5px",borderRadius:4}}>{pt.slice(1,-1)}</code>;
        return pt;
      });
  ...
  return (
    <>
      ...
      <div suppressHydrationWarning style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
        <style suppressHydrationWarning>{`*{box-sizing:border-box}.rcard:hover{transform:translateY(-3px)!important;border-color:rgba(16,185,129,0.35)!important}.rcard{transition:all .2s ease}`}</style>
        <div style={{background:"rgba(0,0,0,0.5)",borderBottom:`1px solid ${T.border}`,padding:"14px 28px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <a href="https://wa.me/971544735060?text=Hi Salman, I want to buy an EA" target="_blank" rel="noopener noreferrer" style={{padding:"7px 16px",background:"linear-gradient(135deg,#10B981,#059669)",borderRadius:20,color:"#000",fontSize:12,fontWeight:700,textDecoration:"none"}}>Buy EA →</a>
          </div>
        </div>
        <div style={{maxWidth:780,margin:"0 auto",padding:"50px 24px"}}>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:18}}>
            <span style={{fontSize:9,color:T.green,background:"rgba(16,185,129,0.08)",border:"1px solid rgba(16,185,129,0.25)",borderRadius:20,padding:"3px 11px",letterSpacing:"0.12em",textTransform:"uppercase",fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{post.tag}</span>
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:36,paddingBottom:24,borderBottom:`1px solid ${T.border}`}}>
            <div style={{width:38,height:38,borderRadius:"50%",background:"linear-gradient(135deg,#10B981,#059669)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17}}>👤</div>
            <div>
          ...
          <div style={{marginTop:48,background:"rgba(16,185,129,0.07)",border:"1px solid rgba(16,185,129,0.2)",borderRadius:16,padding:"24px 28px",textAlign:"center"}}>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:18,fontWeight:700,color:T.green,marginBottom:8}}>🤖 MM FLIP CODEPRO EA</div>
            <p style={{fontSize:13,color:T.sub,marginBottom:18}}>MT4 & MT5 · Parabolic SAR · License protected · 30-day money back · From AED 299</p>
            <a href="https://wa.me/971544735060?text=Hi Salman, I want to buy MM FLIP CODEPRO" target="_blank" rel="noopener noreferrer" style={{display:"inline-block",padding:"12px 30px",background:"linear-gradient(135deg,#10B981,#059669)",borderRadius:24,color:"#000",fontSize:14,fontWeight:700,textDecoration:"none"}}>💬 Buy via WhatsApp →</a>
          </div>
        </div>
        {related.length>0&&(
          <div style={{maxWidth:960,margin:"0 auto",padding:"0 24px 64px"}}>
            <div style={{fontSize:14,fontWeight:700,marginBottom:16,color:"#fff"}}>More EA Articles</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:13}}>
              {related.map(p=>(
                <Link key={p.slug} href={`/salmanfx/blog/${p.slug}`} className="rcard"
