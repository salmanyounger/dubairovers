  ...
  const CARDS = [
    {
      ...
      stats: [{ v:"4.9★", l:"Rating" }, { v:"80+", l:"Countries" }, { v:"10K+", l:"Guests" }],
      ...
      <div suppressHydrationWarning style={{ position:"relative", minHeight:"100vh", background:"#06070d", fontFamily:"'Plus Jakarta Sans',sans-serif", overflowX:"hidden" }}>
        ...
        <div style={{ position:"relative", zIndex:10, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
          ...
          <div style={{ textAlign:"center", marginBottom:"clamp(20px,3.5vh,36px)", animation: mounted ? "fadeUp 0.6s ease both" : "none" }}>
            ...
            <h1 style={{ fontSize:"clamp(32px,5.5vw,64px)", fontWeight:900, color:"#fff", lineHeight:1.05, letterSpacing:"-0.03em", marginBottom:10 }}>
              Dubai Rovers{" "}
              <span style={{ background:"linear-gradient(90deg,#F97316,#ec4899,#818CF8)", backgroundClip:"text", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
                Platform
              </span>
            </h1>
            <p style={{ fontSize:"clamp(12px,1.3vw,15px)", color:"rgba(255,255,255,0.28)", letterSpacing:"0.06em" }}>
              ...
              {small.map(card => (
                <Link key={card.id} href={card.href} className="card-wrap"
                  style={{ flex:1, textDecoration:"none", display:"block", position:"relative", borderRadius:18, overflow:"hidden",
                    ...
                    border:`1px solid ${hovered===card.id ? card.badgeColor+"44" : "rgba(255,255,255,0.06)"}`,
                    boxShadow: hovered===card.id ? `0 20px 50px ${card.glow}` : "0 6px 24px rgba(0,0,0,0.35)" }}
                  onMouseEnter={() => setHovered(card.id)}
                  onMouseLeave={() => setHovered(null)}>
                  <div className="card-img" style={{ position:"absolute", inset:0, backgroundImage:`url('${card.bg}')`, backgroundSize:"cover", backgroundPosition:"center" }}/>
                  <div style={{ position:"absolute", inset:0, background:"linear-gradient(120deg, rgba(4,5,12,0.92) 0%, rgba(4,5,12,0.6) 60%, rgba(4,5,12,0.3) 100%)" }}/>
                  <div style={{ position:"absolute", inset:0, background:`linear-gradient(135deg, ${card.badgeColor}12 0%, transparent 70%)`, opacity:hovered===card.id?1:0.5, transition:"opacity 0.4s" }}/>

                  <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", padding:"0 24px", gap:20 }}>
                    {/* Left */}
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:9, color:card.badgeColor, fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", marginBottom:5 }}>{card.badge}</div>
                      <div style={{ fontSize:"clamp(18px,2.2vw,28px)", fontWeight:900, color:"#fff", letterSpacing:"-0.02em", lineHeight:1.1, marginBottom:4 }}>{card.title}</div>
                      <div style={{ fontSize:10, color:"rgba(255,255,255,0.4)", marginBottom:12 }}>{card.subtitle}</div>
                      <div className="cta-btn" style={{ display:"inline-flex", alignItems:"center", gap:6,
                        background:card.gradient, color:"#fff", padding:"7px 15px", borderRadius:8,
                        fontSize:11, fontWeight:800, boxShadow:`0 3px 14px ${card.glow}` }}>
                        {card.cta} →
                      </div>
                    </div>
                    ...
                    <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                      {card.stats.map(s => (
                        <div key={s.l} style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:8, padding:"8px 10px", textAlign:"center",
                          transform: hovered===card.id ? "translateY(-2px)" : "none", transition:"transform 0.3s ease" }}>
                          <div style={{ fontSize:14, fontWeight:900, color:"#fff" }}>{s.v}</div>
                          <div style={{ fontSize:8, color:"rgba(255,255,255,0.3)", textTransform:"uppercase", letterSpacing:"0.05em" }}>{s.l}</div>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div style={{ marginTop:"clamp(10px,2vh,20px)", textAlign:"center", animation: mounted ? "fadeUp 1s ease 0.25s both" : "none" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:16, flexWrap:"wrap" }}>
              {["24/7 Support","Free Tools","AI Powered","DLD Verified","WhatsApp +971544735060"].map(t => (
