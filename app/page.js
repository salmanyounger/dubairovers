...
const PLATFORMS = [
  { id:"tours", tag:"TOURS & EXPERIENCES", tagColor:"#F59E0B", title:"Dubai Rovers Adventures", desc:"Desert safaris, hot air balloons, dhow cruises — hotel pickup on every tour.", emoji:"🏜️", bgGradient:"linear-gradient(135deg,#1a0800,#2d1500)", stats:[{num:"4.9★",lbl:"RATING"},{num:"80+",lbl:"COUNTRIES"},{num:"24",lbl:"TOURS"}], href:"/tours", category:"tours" },
  { id:"properties", tag:"REAL ESTATE · AI", tagColor:"#00BFFF", title:"Dubai Properties", desc:"30+ UAE properties, AI scoring, mortgage calculators, market trends & ROI.", emoji:"🏢", bgGradient:"linear-gradient(135deg,#00102a,#001f4a)", stats:[{num:"30+",lbl:"PROPERTIES"},{num:"AI",lbl:"SCORING"}], href:"/properties", category:"properties" },
  { id:"archai", tag:"AI VILLA DESIGN", tagColor:"#C9A84C", title:"ARCHAI Villa Designer", desc:"Design your dream villa in 3 minutes — 9 styles, floor plans, cost estimate.", emoji:"🏛️", bgGradient:"linear-gradient(135deg,#0f0a00,#2a1f00)", stats:[{num:"9",lbl:"STYLES"},{num:"Free",lbl:"UNTIL 2026"}], href:"/archai", category:"ai" },
  ...
  { id:"webbuilder", tag:"WEB DEV · DUBAI", tagColor:"#A855F7", title:"Web Builder", desc:"18+ business categories, 5 style tiers, multilingual, booking forms & admin.", emoji:"💻", bgGradient:"linear-gradient(135deg,#1a001a,#3d0033)", stats:[{num:"18+",lbl:"CATEGORIES"},{num:"Fast",lbl:"DELIVERY"}], href:"/webbuilder", category:"webdev" },
...
function MobileLanding() {
  ...
  const filtered = activeCategory === "All Platforms" ? PLATFORMS : PLATFORMS.filter(p => p.category === CAT_MAP[activeCategory]);

  return (
    <div suppressHydrationWarning style={{ maxWidth:"430px", margin:"0 auto", minHeight:"100vh", background:"#080808", color:"#fff", fontFamily:"'Plus Jakarta Sans',sans-serif", overflowX:"hidden" }}>
      ...
      <div style={{padding:"8px 18px 20px",textAlign:"center"}}>
        <div style={{display:"inline-block",background:"rgba(245,158,11,0.1)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:20,padding:"5px 14px",fontSize:9,fontWeight:700,letterSpacing:"2px",color:"#F59E0B",textTransform:"uppercase",marginBottom:14}}>Welcome to</div>
        ...
        <p style={{fontSize:13,color:"rgba(255,255,255,0.5)",maxWidth:280,margin:"0 auto 20px"}}>Tours · Properties · AI Design · Forex EA · Web Dev</p>
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <Link href="/tours" style={{flex:1,maxWidth:160,display:"flex",alignItems:"center",justifyContent:"center",padding:"13px",background:"linear-gradient(135deg,#F59E0B,#f97316)",borderRadius:12,color:"#000",fontSize:13,fontWeight:700,textDecoration:"none"}}>Explore All →</Link>
          <a href="https://wa.me/971544735060" target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"center",gap:6,padding:"13px 16px",background:"rgba(37,211,102,0.1)",border:"1px solid rgba(37,211,102,0.3)",borderRadius:12,color:"#25d366",fontSize:13,fontWeight:700,textDecoration:"none"}}>💬 WhatsApp</a>
        </div>
      </div>
      {/* CATEGORY FILTER */}
      <div style={{overflowX:"auto",padding:"4px 18px",display:"flex",gap:8,scrollbarWidth:"none"}}>
        {CATEGORIES.map(cat => (
          <button key={cat} className="cat-pill" onClick={() => setActiveCategory(cat)}
            style={{flexShrink:0,padding:"8px 16px",borderRadius:20,border:"1px solid",borderColor:activeCategory===cat?"rgba(245,158,11,0.3)":"rgba(255,255,255,0.1)",background:activeCategory===cat?"rgba(245,158,11,0.12)":"transparent",color:activeCategory===cat?"#F59E0B":"rgba(255,255,255,0.6)",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
            {cat}
          </button>
        ))}
      </div>
      <div style={{padding:"8px 18px",fontSize:11,color:"rgba(255,255,255,0.35)"}}>Showing {filtered.length} platform{filtered.length!==1?"s":""}</div>
      {/* CARDS */}
      <div style={{padding:"0 18px 24px",display:"flex",flexDirection:"column",gap:14}}>
        {filtered.map((p,i) => (
          <Link key={p.id} href={p.href} className="card-link" style={{textDecoration:"none"}}>
            <div className="card-anim" style={{background:"#111111",border:"1px solid rgba(255,255,255,0.07)",borderRadius:20,overflow:"hidden",display:"flex",transition:"transform 0.2s ease",animationDelay:`${i*60}ms`}}>
              <div style={{width:110,minHeight:130,flexShrink:0,background:p.bgGradient,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
              ...
              <div style={{padding:"16px 14px",flex:1,display:"flex",flexDirection:"column",justifyContent:"space-between",minWidth:0}}>
                <div>
                  ...
                  <div style={{fontSize:15,fontWeight:800,color:"#fff",letterSpacing:"-0.2px",marginBottom:6,lineHeight:1.3}}>{p.title}</div>
                  ...
                  <div style={{display:"flex",gap:14}}>
                    {p.stats.map(s => (
                      <div key={s.lbl}>
                        ...
                        <div style={{fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.35)",textTransform:"uppercase",marginTop:1}}>{s.lbl}</div>
                      </div>
                    ))}
                  </div>
      ...
      <div style={{padding:"24px 18px 40px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:8}}><span style={{fontSize:11,color:"rgba(255,255,255,0.25)"}}>© 2026 DubaiRovers.com · All Rights Reserved</span></div>
