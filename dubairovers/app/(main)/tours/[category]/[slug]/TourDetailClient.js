      ...
      <div className="container-main py-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-5 flex-wrap text-sm">
                ...
                <span className="text-gray-500">📅 {tour.bookingCount?.toLocaleString()} bookings</span>
                <span className="text-gray-300">|</span>
              ...
              <div className="animate-fade-in">
                <div className="flex items-center gap-6 mb-8 p-6 rounded-2xl bg-amber-50">
                  <div className="text-center">
                    ...
                    <div className="text-sm text-gray-500 mt-1">{tour.reviewCount?.toLocaleString()} reviews</div>
                  </div>
          ...
          <div className="lg:w-80 shrink-0">
            <div className="sticky top-24 rounded-3xl overflow-hidden"
              ...
              <div className="p-6 text-white" style={{ background:'linear-gradient(135deg,#0A1628,#1E3A5F)' }}>
                ...
                <div className="text-4xl font-black">{convert(tour.pricing?.adult)}
                  <span className="text-base font-normal text-white/60"> / person</span>
                </div>
                {currency.code !== 'AED' && (
                  <div className="text-white/40 text-xs mt-1">≈ AED {tour.pricing?.adult} per person</div>
                )}
                <div className="flex items-center gap-1.5 mt-2">
                  <Stars rating={tour.rating} />
                  <span className="text-amber-400 font-bold text-sm">{tour.rating}</span>
                  <span className="text-white/50 text-xs">({tour.reviewCount?.toLocaleString()})</span>
                </div>
              </div>

              <div className="p-4 grid grid-cols-2 gap-2" style={{ background:'#fafafa', borderBottom:'1px solid #f0f0f0' }}>
                {[
                  { label:'Duration', value:tour.duration, emoji:'⏱️' },
                  { label:'Group',    value:`Max ${tour.groupSize?.max}`, emoji:'👥' },
                  { label:'Language', value:tour.languages?.[0]||'English', emoji:'🌍' },
                  { label:'Cancel',   value:'Free 24h', emoji:'🔄' },
                ].map(info => (
                  <div key={info.label} className="text-center p-2 rounded-xl bg-white">
                    <div className="text-lg mb-0.5">{info.emoji}</div>
                    <div className="text-[10px] text-gray-400">{info.label}</div>
                    <div className="text-xs font-bold text-brand-navy">{info.value}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="p-5 space-y-3">
                <button onClick={() => {
                  setShowBooking(s => {
                    if (!s) setTimeout(() => document.getElementById('booking-form-section')?.scrollIntoView({ behavior:'smooth', block:'start' }), 100);
                    return !s;
                  });
                }} className="btn-gold w-full justify-center py-4 text-base">
                  📋 Book This Tour
                </button>
                <a href={`https://wa.me/971544735060?text=Hello! I'm interested in: *${tour.name}*. Can you help me book?`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full py-3.5 rounded-full font-bold text-white text-sm"
                  style={{ background:'#25D366' }}>
                  💬 WhatsApp Enquiry
                </a>
                <a href={`mailto:dbtis.com@gmail.com?subject=Enquiry: ${tour.name}`}
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-full font-semibold text-gray-600 text-sm border border-gray-200 hover:border-brand-gold hover:text-brand-gold transition-colors">
                  📧 Email Enquiry
                </a>
                <div className="text-center text-xs text-gray-400 pt-1">
                  🔒 Secure · ⚡ Confirmed in 30 min · 🔄 {tour.cancellation}
                </div>
              </div>
            </div>

            {tour.addOns?.length > 0 && (
              <div className="mt-4 rounded-2xl p-4 border border-gray-100 bg-white">
                <h4 className="font-bold text-brand-navy text-sm mb-3">✨ Add-ons Available</h4>
                {tour.addOns.map(a => (
                  <div key={a.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                    <span className="text-sm text-gray-600">{a.emoji} {a.name}</span>
                    <span className="text-sm font-bold text-brand-navy">+AED {a.price}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {showBooking && (
          <div
            ...
            className="mt-8 rounded-3xl border border-gray-100"
            style={{ boxShadow:'0 8px 48px rgba(10,22,40,0.1)', paddingBottom:'80px' }}>
