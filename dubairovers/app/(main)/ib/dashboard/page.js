      ...
      <div className="container-main py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { emoji:'💰', label:'Total Revenue',     value:`AED ${MOCK_IB.revenue.toLocaleString()}`,   color:'#D4AF37' },
            { emoji:'💸', label:'Commission Earned', value:`AED ${MOCK_IB.commission.toLocaleString()}`,color:'#10B981' },
            { emoji:'⏳', label:'Pending Payout',    value:`AED ${MOCK_IB.pending.toLocaleString()}`,   color:'#F59E0B' },
          ].map(s => (
                ...
                {MOCK_IB.tours_list.filter(t=>t.revenue>0).map(t => (
                  <div key={t.name}>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="font-bold text-brand-gold">AED {t.revenue.toLocaleString()}</span>
                    </div>
          ...
          <div className="max-w-lg space-y-5">
            <div className="rounded-2xl bg-white p-6" style={{ boxShadow:'0 2px 12px rgba(10,22,40,0.06)', border:'1px solid #e2e8f0' }}>
              ...
              <div className="space-y-3 text-sm">
                {[
                  ['Total Revenue',     `AED ${MOCK_IB.revenue.toLocaleString()}`],
                  ['Platform Fee (20%)',`−AED ${(MOCK_IB.revenue*0.2).toLocaleString()}`],
                  ['Your Share (80%)',  `AED ${(MOCK_IB.revenue*0.8).toLocaleString()}`],
                  ['Already Paid',     `−AED ${(MOCK_IB.commission).toLocaleString()}`],
                ].map(([k,v])=>(
                ...
                <div className="flex justify-between pt-1">
                  ...
                  <span className="font-black text-emerald-600 text-xl">AED {MOCK_IB.pending.toLocaleString()}</span>
                </div>
              </div>
            </div>
            <button onClick={() => {
