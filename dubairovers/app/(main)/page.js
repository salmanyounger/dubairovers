...
function StatCard({ stat }) {
  ...
  return (
    <div ref={ref} className="text-center">
      ...
      <div className="text-3xl md:text-4xl font-black text-brand-gold">
        {stat.suffix === '★' ? stat.value : count.toLocaleString()}{stat.suffix}
      </div>
    ...
    <>
      ...
      <section className="section-pad bg-white">
        <div className="container-main">
          <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
            <div>
              <span className="badge-gold mb-3">🌍 Worldwide</span>
              <h2 className="text-3xl md:text-4xl font-bold text-brand-navy mt-3" style={{ fontFamily:"'Playfair Display',serif" }}>
                Go Beyond Dubai
              </h2>
              <p className="text-gray-600 mt-2">Explore our worldwide tour packages departing from Dubai</p>
            </div>
            <Link href="/tours/worldwide-tours" className="btn-outline text-sm">View All Destinations →</Link>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {WORLD_DESTINATIONS.map(dest => (
              <Link key={dest.id} href={`/tours/worldwide-tours?dest=${dest.id}`}
                className="group relative overflow-hidden rounded-2xl cursor-pointer"
                style={{ aspectRatio:'3/4' }}>
                <Image src={dest.image} alt={dest.name} fill
                  className="object-cover transition-transform duration-500 group-hover:scale-110" />
                <div className="absolute inset-0" style={{ background:'linear-gradient(180deg,transparent 40%,rgba(10,22,40,0.9) 100%)' }} />
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  ...
                  <div className="text-brand-gold text-xs font-bold mt-1">From AED {dest.fromPrice.toLocaleString()}</div>
                </div>
              </Link>
            ))}
          </div>
