            ...
            <tbody>
              {inv.lines.map((l,i)=>(
                <tr key={i} className="border-b border-gray-100">
                  ...
                  <td className="py-3 text-right text-gray-600">AED {(+l.unit).toLocaleString()}</td>
                  ...
                  <td className="py-3 text-right font-semibold">AED {((+l.qty||0)*(+l.unit||0)).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr><td colSpan={99} className="border-t pt-3"></td></tr>
              <tr><td colSpan={5} className="text-right text-sm text-gray-500 pb-1">Subtotal</td><td className="text-right font-semibold pb-1">AED {sub.toLocaleString()}</td></tr>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl p-4 text-sm" style={{background:'rgba(212,175,55,0.06)',border:'1px solid rgba(212,175,55,0.2)'}}>
        <div className="flex justify-between mb-1"><span className="text-gray-500">Subtotal</span><span className="font-semibold">AED {sub.toLocaleString()}</span></div>
